...
WorkCalendar.prototype = {
	...
	generateMonth: function(month) {
		var html = '<div class="datepicker dropdown-menu span3" style="display: block; position: relative; margin-right:20px;">'
		...
		for (var j = 0; j < lastDayOfMonth; j++) {
			...
			html += '<td class="week' + week + ' date' + date + '" style="border:solid black 1px;">' + (j + 1) + '</td>';
		}
...
+'</div>';
	...
	markHolidays: function(holidays) {
		for (var i = 0; i < holidays.length; i++) {
			var item = holidays[i];
			var day = new Date(item.workDate);
			var currentDay = day.getFullYear() + '' + (day.getMonth()+1) + ''+ day.getDate();
			$('.date' + currentDay).css({
				backgroundColor: '#33CC66'
			});
			$('.date' + currentDay).attr('title', item.name);
		}
	},
